...
vectorizer = TfidfVectorizer(stop_words=stpwrdlst, lowercase=True, max_features=3000)
# 训练集对应稀疏矩阵
...
class NaiveBayes:
    ...
    def train(self, x, y):
        ...
        for i in range(self.classes):
            ...
            m = np.array(m).T  # 将样本矩阵转置
            for j in range(x.shape[1]):  # 遍历所有特征
                sum = np.sum(m[j])
                # 求出条件概率pxy
                self.Pxy[i][j] = (100000.0) * (sum + 1) / (sum_m + x.shape[1])
...
if __name__ == "__main__":
    ...
    model = NaiveBayes(num_class=class_nums)
    # 训练模型
